...
from talentarc_core.compliance.network_blocker import network_disabled
...
def main():
    ...
    if "anthropic" in sys.modules:
        raise RuntimeError("DISQUALIFIER: Anthropic SDK was loaded during offline ranking.")
    ...
    # WRAP ENTIRE PIPELINE IN NETWORK BLOCKER
    with network_disabled():
        ...
        if os.path.exists(llm_cache_file):
            ...
            print("[Rank] WARNING: llm_features.parquet not found. Falling back to median/0.5 multipliers.")
        ...
        submission_df.to_csv(".submission_raw.csv", index=False)
        
        elapsed = time.time() - start_time
        print(f"[Rank] Finished successfully in {elapsed:.2f} seconds.")
        print("Files created: .submission_raw.csv, compliance_audit.json")
